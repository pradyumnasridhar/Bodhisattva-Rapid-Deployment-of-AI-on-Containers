...
def activeUsers():
	...
	return jsonify({
		"status": "success",
		"message": int(result["ActiveUsers"])
	})
...
def totalPredictions():
	...
	return jsonify({
		"status": "success",
		"message": int(result["TotalPredictions"])
	})
...
def predictorInstances():
	...
	query = """
		select Count(*) as PredictorInstances
		from Predictors
...
@app.route('/api/get-tenant-components', methods=['POST'])
def getTenantComponents():
	tenantId = request.json["tenantId"]

	query = """
		select ComponentId, ComponentName, IPAddress
		from Components
		where TenantId = {}
	""".format(tenantId)
...
# ---------------------------UI----------------------------------------
def getSingleModelDetails(modelId):
	query = """
		select ModelId, ModelName, Description
		from Models
		where ModelId={};
	""".format(str(modelId))
...
@app.route('/predictors-details', defaults={"modelId": 0})
@app.route('/predictors-details/', defaults={"modelId": 0})
@app.route('/predictors-details/<int:modelId>')
def predictorsDetails(modelId):
	return render_template('predictors_details.html', modelId=modelId)

@app.route('/webapp-builder-wizard')
def webappBuilderWizard():
	return render_template('webapp_builder_wizard.html')

@app.route('/model-builder-wizard', defaults={"modelId": 0})
@app.route('/model-builder-wizard/', defaults={"modelId": 0})
@app.route('/model-builder-wizard/<int:modelId>')
def modelBuilderWizard(modelId):
	return render_template('model_builder_wizard.html', modelId=modelId)

@app.route('/deploy-predictors/<int:modelId>', methods=["GET"])
def deployPredictors(modelId):
	modelDetails = getSingleModelDetails(modelId)
	return render_template('deploy_predictors.html', modelId=modelDetails["ModelId"], modelName=modelDetails["ModelName"], modelDescription=modelDetails["Description"])
...
if __name__ == '__main__':
	app.run(debug=True, host='0.0.0.0', port=meta["AppPort"], use_reloader=False)
